...
class ParseConfig
{
     ...
     * @param bool     $useInclude [Optional] As of PHP 5 the FILE_USE_INCLUDE_PATH
     *                             constant can be used to trigger include path
     *                             search.
     * @param resource $context    [Optional] A valid context resource created
     *                             with stream_context_create. If you don't need
     *                             to use a custom context, you can skip this 
     *                             parameter by NULL.
     * @param int      $offset     [Optional] The offset where the reading 
     *                             starts on the original stream. Seeking 
     *                             (offset) is not supported with remote files. 
     *                             Attempting to seek on non-local files may 
     *                             work with small offsets, but this is 
     *                             unpredictable because it works on the 
     *                             buffered stream.
     * @param int      $maxlen     [Optional] Maximum length of data read. The 
     *                             default is to read until end of file is 
     *                             reached. Note that this parameter is applied
     ...
     */
    public static function fget_contents($filename, $useInclude = false,
        $context = null, $offset = -1, $maxlen = null
    ) {
        $args = array($filename, $useInclude, $context, $offset, $maxlen);
